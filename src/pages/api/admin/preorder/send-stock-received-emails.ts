...
import { getStockReceivedEmail } from "../../../../lib/emailTemplates";
...
export const POST: APIRoute = async ({ request }) => {
	...
	for (const [userId, orderId] of orderByUser.entries()) {
		...
		const emailContent = getStockReceivedEmail({
		...
		try {
			...
			console.error("Stock email failed", error);
		}
	}
